...
def extract_shape_at_frame(img_path, num_points=36):
...
# 7 Exact Frames from Video
shape_indices = [0, 40, 80, 120, 160, 200, 240]

for idx in shape_indices:
    pts = extract_shape_at_frame(f'scratch/video_frames/exact_shape_{idx:03d}.png', 36)
    if pts:
        path_str = pts_to_svg_path(pts)
        print(f'SHAPE_{idx:03d} = "{path_str}"')
